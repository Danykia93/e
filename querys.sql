...
-- Mapeamento de Estoques em Brasília, a partir de uma condição
SELECT * FROM warehouses
	WHERE location = 'DF';
...
-- Abaixo de 12 = Estoque Baixo
-- Igual a 12 = Estoque no Limite
-- Acima de 12 = Estoque Alto
SELECT s.Sname, s.ZipCode, p.Pname, ps.SQuantity
	CASE
    	WHEN ps.SQuantity < 12 THEN 'Estoque Baixo'
        WHEN ps.SQuantity = 12 THEN 'Estoque Limite'
    	ELSE 'Estoque Alto'
